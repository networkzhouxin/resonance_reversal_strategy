...
当前策略文件为 build `20260902.4` 候选：只新增 09:35 固定 3% 止盈退出。当前价相对
JoinQuant 持仓 `avg_cost` 的收益率达到或超过 3% 时，独立提交清仓；未成交退出会冻结为
`TAKE_PROFIT_EXIT` 并在下一交易日优先重试。该候选尚无 JoinQuant 真实路径结果，不能
替代上述 `.3` 保留基线。本候选不包含最长持有 5 个交易日或任何其他持有期退出。
...
- T 日现价只用于可交易状态、固定止盈检查、ATR 观察、下单和订单后实际持仓核对，不能反向修改
  T-1 信号、候选顺序或仓位公式。
...
`20260902.3` 当前保留基线、`.4` 待验证候选身份及策略规范最后的覆盖条款为准。
