...
本目录是一套独立的聚宽策略。当前风险控制型保留基线为 build `20260904.2`，策略
行为提交为 `2860317`。用户已在审阅 2019—2021 普通摩擦 JoinQuant 真实路径结果后
明确采用该版本；后续交易候选只能从该基线派生。双倍摩擦仍为
`DEFERRED_NOT_RUN`，当前保留不代表实盘授权或验证期已经开放。

当前策略文件为从该基线派生的 build `20260904.6` CCI14 观察采集版。它只把
T-1 `CCI(14)` 写入 `observation_values`，没有取得任何买卖、排序或仓位资格；在
聚宽真实路径验证和用户再次确认前，保留基线仍是 build `20260904.2`。

## 策略摘要

策略不计算综合分数。BOLL 必须提供价格结构方向，RSI14 或 KDJ 至少一个同向时仍可
构成完整共振观察；若第三个指标出现反向事件，则否决该次共振。当前保留基线进一步
要求 FORMAL 和 RELATIVE 的新买入必须由 BOLL、RSI、KDJ 三项全部同向支持；RELATIVE
还只允许 `SOFT_ALL_THREE` 分支获得新买入资格，`HARD_BOLL_SOFT_OSC` 保留观察但不得
进入买入排序或订单。两指标完整共振继续登记观察。事件有效窗口为两个交易日，并且
...
95%。RSI6/12/24、ADX/+DI/-DI、成交量、量比、BOLL 带宽、中轨斜率和CCI14仅记录观察，
...
`20260904.2` 当前保留基线及策略规范最后的覆盖条款为准。
