...
本目录是一套独立的聚宽策略。当前保留基线为 build `20260902.3`，策略行为提交为
`350e3dd`。用户已在审阅 2019—2021 普通摩擦 JoinQuant 真实路径结果后明确采用该
版本；后续交易候选只能从该基线派生。双倍摩擦仍为 `DEFERRED_NOT_RUN`，当前保留
不代表实盘授权或验证期已经开放。

当前策略文件为 build `20260902.5` 候选。它保留 `.4` 的 09:35 固定 3% 止盈，并仅新增
“持仓达到第 5 个交易日后，当前价相对 JoinQuant 持仓 `avg_cost` 亏损达到或超过 3% 时
清仓”。5 个交易日只控制这条亏损退出；未达到亏损阈值时可以继续持有任意时长，不是最长
持有期。用户已根据普通摩擦 JoinQuant 收益截图明确拒绝 `.5`；策略文件仍保留该失败实验
以便复现，不得作为后续基线。`.4` 仍未回测，二者都不能替代上述 `.3` 保留基线。

## 策略摘要

策略不计算综合分数。BOLL 必须提供价格结构方向，RSI14 或 KDJ 至少一个同向时仍可
构成完整共振观察；若第三个指标出现反向事件，则否决该次共振。当前保留基线进一步
要求 FORMAL 和 RELATIVE 的新买入必须由 BOLL、RSI、KDJ 三项全部同向支持。两指标
完整共振继续登记观察，但不得进入买入排序或订单。事件有效窗口为两个交易日，并且
至少一个支持事件必须发生在 T-1。ATR14、止损边界和最高收盘锚继续计算与记录，但
`ATR_EXIT_POLICY="OBSERVE_ONLY"`，不提交 ATR 卖单。
...
- T 日现价只用于可交易状态、固定止盈/亏损检查、ATR 观察、下单和订单后实际持仓核对，不能反向修改
  T-1 信号、候选顺序或仓位公式。
...
下列章节保留各 build 的演进历史；若历史候选描述与当前行为不同，以本文顶部的
`20260902.3` 当前保留基线、`.4/.5` 待验证候选身份及策略规范最后的覆盖条款为准。
