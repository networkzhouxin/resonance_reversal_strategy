...
def boll_kdj_sell_snapshot(code, signal_date="2021-01-05"):
    snapshot = resonance_snapshot(
        code, direction="SELL_TURN", signal_date=signal_date,
    )
    snapshot["event_book"] = event_book_for_directions(
        "SELL_TURN", "NEUTRAL", "SELL_TURN", signal_date,
    )
    return snapshot
...
def test_boll_kdj_only_sell_is_not_delayed(monkeypatch):
    ...
    monkeypatch.setattr(
        strategy, "submit_sell",
        lambda context_arg, order_code, reason, trigger_value:
